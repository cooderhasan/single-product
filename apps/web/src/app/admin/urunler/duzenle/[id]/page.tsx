...
export default function EditProductPage() {
  ...
  const fetchProduct = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Debug: API'den gelen ürün görsellerini kontrol et
        console.log('API response product.images:', JSON.stringify(product.images));
        
        // API'den gelen görseller: ProductImage[] olarak gelebilir [{url: "...", isMain: true, ...}]
        const images = Array.isArray(product.images) 
          ? product.images.map((img: string | { url: string }) => {
              console.log('Processing image:', img, typeof img);
              return typeof img === 'string' ? img : (img?.url || '');
            }).filter(Boolean) // Boşları filtrele
          : [];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log('Sending images to API:', JSON.stringify(formData.images));
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-gray-500">Ürün bilgilerini güncelle</p>
        </div>
      </div>
            ...
            {formData.images.length > 0 && (
              <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-4">
                {formData.images.map((img, index) => {
                  ...
                  const imgSrc = img.startsWith('http') ? img : `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3041'}${img}`;
                  return (
